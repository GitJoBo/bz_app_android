...
/**
 * @Desc: 目的：用list给一个view设置不同的样式<br />
 * 包括：1. 字体的颜色和大小，2. 点击效果
 * 使用：TextView textView=new TextView(context);
...
public class TextParserUtils {
     ...
     * @param size     字体大小px
     * @param color    字体颜色
     * @param typeface 字体模式，加粗，倾斜等
     * @return
     */
    public TextParserUtils append(String text, int size, int color, int typeface) {
        if (text == null) {
            return this;
        }
        TextBean bean = new TextBean();
    ...
    public void parse(TextView textView) {
        ...
        for (TextBean bean : textList) {
            ...
            //加斜等样式
            if (bean.typeface != 0) {
                style.setSpan(new StyleSpan(bean.typeface), position, position
                        + bean.text.length(), Spannable.SPAN_EXCLUSIVE_INCLUSIVE);
            }
            /*
